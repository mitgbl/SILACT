...
def CreateDataset(Train_idxes,Test_idxes,str1,str2):
    ...
    for idx in Train_idxes:
        ...
        if (os.path.exists(inputpath)) is False or (os.path.exists(gtpath)) is False:
            ...
            with h5py.File(gtpath, 'r') as Outputs:
                dn = (Outputs['n_high'][()])
                gt = dn#np.transpose(dn,(2,0,1)) # (100, 256, 256)
    ...
    for ii in Test_idxes:
        ...
        if (os.path.exists(inputpath)) is False or (os.path.exists(gtpath)) is False:
            ...
            with h5py.File(gtpath, 'r') as Outputs:
                ...
                dn = (Outputs['n_high'])
                gt1 = dn#np.transpose(dn,(2,0,1)) # (100, 256, 256)
            # Outputs = scio.loadmat(gtpath) #output_path
            # dn = (Outputs['n_high'])
...
# import data
    ...
    str2 = 'C:\\Users\\Quadro\\MIT 3D Optics Group Dropbox\\Deep-learning_ODT\\Training_Dataset\\GT_high_freq'
...
# make direction for training results
    resultFolder = 'C:\\Users\\Quadro\\MIT 3D Optics Group Dropbox\\Deep-learning_ODT\\pytorch_script\\pred_result\\Pred_high_freq'
